...
interface RomTable {
  ...
  [tableNameOrIndex: string | number]:
    | RomTable
    | string
    | Axis
    | number[][]
...
export class Rom {
  ...
  fillTable(table) {
    ...
    for (let i = 0; i < elements; i++) {
      ...
      switch (table.type) {
        ...
        default:
          console.log(
            `cannot fill table type ${table.type} tableName: ${table.name}`
          );
      }
    }
    return table;
  ...
  PrintTable({
    ...
    let tableCellWidth = Math.max(
      ...tableValues.flat().map((x) => {
        // if (formatter) {
        //     x = formatter(x)
        // }
        return `${x}`.length;
    ...
    if (table.xAxis) {
      tableCellWidth = Math.max(
        tableCellWidth,
        ...table.xAxis.values.map((x) => {
          // if (formatter) {
          //     x = formatter(x)
          // }
          return `${x}`.length;
    ...
    tableValues.forEach((y, yIndex) => {
      ...
      if (table.yAxis && !noAxis) {
        tableAsString += `${table.yAxis.values[yIndex]}`.padStart(
          tableCellWidth
        );
      }
      y.forEach((x) => {
  ...
  FillLogTable({
    ...
    switch (table.type) {
      ...
      default:
        console.log(
          `PrintLogTable: unknown table type ${table.type} on tableName: ${tableName}`
        );
    }
  }
    ...
    realToTableValues.forEach((row, y) => {
      ...
      row.forEach((cellValue, x) => {
        ...
        let axes = this.getAxes(
          realFromTable,
          newAxisValues.xValue,
          newAxisValues.yValue
        );
